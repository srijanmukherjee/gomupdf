...
// gomupdf_list_widgets returns a malloc'd string with one line per widget:
//   typeName\tx0\ty0\tx1\ty1\tValue\tName
// Returns NULL+err on failure, or "" if no widgets.
static char *gomupdf_list_widgets(fz_context *ctx, fz_document *doc, int pageno,
    ...
    fz_try(ctx) {
        ...
        for (w = pdf_first_widget(ctx, page); w; w = pdf_next_widget(ctx, w)) {
            ...
            // Get value from the field object
            pdf_obj *fobj = pdf_annot_obj(ctx, w);
            const char *val = pdf_field_value(ctx, fobj);
            if (!val) val = "";

            // Get field name - pdf_load_field_name returns malloc'd string we must free
            char *name = pdf_load_field_name(ctx, fobj);

            // sanitize name/val (strip tabs/newlines)
            char safe_name[512];
            if (name) {
                size_t ni = 0;
...
// gomupdf_set_text_field sets the text value of the widget at index on pageno.
...
// gomupdf_set_checkbox sets the checked state of the checkbox widget at index.
// Returns 0 on success, -1 on error.
static int gomupdf_set_checkbox(fz_context *ctx, fz_document *doc, int pageno,
    ...
    fz_try(ctx) {
        ...
        // Determine the on-state name from the field object
        pdf_obj *fobj = pdf_annot_obj(ctx, w);
...
// gomupdf_add_text_field creates a text AcroForm field named `name` over `rect`
// with initial value `value` on page `pageno`. Returns 0 on success, -1 on error.
static int gomupdf_add_text_field(fz_context *ctx, fz_document *doc, int pageno,
    ...
    fz_try(ctx) {
        ...
        // Ensure AcroForm with /Fields array and default appearance resources exist.
        ...
        if (!fields || !pdf_is_array(ctx, fields)) {
            ...
            // Keep a local reference (it's now owned by acro) — re-get it
            fields = pdf_dict_get(ctx, acro, PDF_NAME(Fields));
        }

        // Default appearance if not present
        ...
        // Set initial value via the high-level API (builds appearance)
...
func (p *Page) Widgets() ([]Widget, error) {
	...
	cstr := C.gomupdf_list_widgets(d.ctx, d.doc, C.int(p.Number), errBuf, errBufLen)
	if cstr == nil {
		return nil, errors.New("gomupdf: widgets: " + C.GoString(errBuf))
	}
...
func (p *Page) SetTextField(index int, value string) error {
	...
	errBuf := (*C.char)(C.malloc(errBufLen))
	defer C.free(unsafe.Pointer(errBuf))
	if C.gomupdf_set_text_field(d.ctx, d.doc, C.int(p.Number), C.int(index), cvalue, errBuf, errBufLen) != 0 {
...
func (p *Page) SetCheckbox(index int, checked bool) error {
	...
	errBuf := (*C.char)(C.malloc(errBufLen))
	defer C.free(unsafe.Pointer(errBuf))
	if C.gomupdf_set_checkbox(d.ctx, d.doc, C.int(p.Number), C.int(index), cChecked, errBuf, errBufLen) != 0 {
...
func (p *Page) AddTextField(name string, rect geometry.Rect, value string) error {
	...
	errBuf := (*C.char)(C.malloc(errBufLen))
	defer C.free(unsafe.Pointer(errBuf))
	if C.gomupdf_add_text_field(d.ctx, d.doc, C.int(p.Number),
		cname,
