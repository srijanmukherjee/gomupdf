...
// embedded is "1" or "0". Encoding is "" when absent or not a name object.
// Returns a malloc'd NUL-terminated string (caller must free) or NULL+err on error.
// Returns an empty string (still malloc'd) when there are no fonts.
static char *gomupdf_page_fonts(fz_context *ctx, fz_document *doc, int pageno,
    ...
    fz_try(ctx) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // Capture the xref from the indirect reference before resolving.
            ...
            // Locate FontDescriptor: for Type0 it lives on DescendantFonts[0].
            pdf_obj *desc = NULL;
...
// On success: returns a malloc'd byte buffer (caller must free), sets *out_len,
// sets ext_out (8 bytes) to "ttf"/"cff"/"otf"/"pfa", and sets name_out (256 bytes) to BaseFont.
// When not embedded: returns NULL, *out_len=0, ext_out="", errBuf unchanged (no error).
// On real error: returns NULL and writes into errBuf.
    ...
    *out_len = 0;
    ...
    fz_try(ctx) {
        ...
        if (desc) {
            ...
            if (stream) {
                // pdf_load_stream accepts both indirect refs and direct stream dicts.
                // Pass it as-is (keep the indirect ref when present so MuPDF resolves internally).
...
func (p *Page) GetFonts() ([]FontInfo, error) {
	...
	*errBuf = 0
	...
	cstr := C.gomupdf_page_fonts(d.ctx, d.doc, C.int(p.Number), errBuf, errBufLen)
	if cstr == nil {
		msg := C.GoString(errBuf)
		if msg == "" {
			return nil, nil
		}
		return nil, errors.New("gomupdf: get fonts: " + msg)
	}
